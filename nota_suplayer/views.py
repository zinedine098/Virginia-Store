...
def nota_kosong(request):
    ...
    temp_nota_id = request.session.get('temp_nota_id_suplayer')
    if not temp_nota_id:
        # Create a temporary NotaPayment with dummy data (no suplayer yet)
        temp_nota = NotaPayment.objects.create(
            suplayer=None,  # No suplayer initially
        ...
        request.session['temp_nota_id_suplayer'] = temp_nota_id
    else:
        try:
    ...
    if request.method == 'POST':
        if 'add_item' in request.POST:
            ...
            if payment_form.is_valid():
                # Get items from DB
                items = NotaSuplayer.objects.filter(nota_payment=temp_nota)
                if not items.exists():
                ...
                temp_nota.suplayer = payment_form.cleaned_data['suplayer']
                ...
                temp_nota.save()
                # Clear session
                if 'temp_nota_id_suplayer' in request.session:
                    del request.session['temp_nota_id_suplayer']
    ...
    daftar_barang = []
    for item in NotaSuplayer.objects.filter(nota_payment=temp_nota):
...
def update_quantity(request, item_id):
    if request.method == 'POST':
        try:
            ...
            db_item = NotaSuplayer.objects.get(id=item_id)
        ...
        except NotaSuplayer.DoesNotExist:
...
def delete_item(request, item_id):
    if request.method == 'POST':
        try:
            ...
            NotaSuplayer.objects.filter(id=item_id).delete()
...
def edit_nota(request, nota_id):
    ...
    # Load daftar_barang from DB
    daftar_barang = []
    for item in NotaSuplayer.objects.filter(nota_payment=nota):
        daftar_barang.append({
            'id': str(item.id),
            'db_id': item.id,
            'kode_barang': item.kode_barang,
